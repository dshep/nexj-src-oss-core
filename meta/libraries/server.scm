...
(import 
   '( nexj.core.rpc.text.TextMarshaller
      ...
      nexj.core.monitoring.Average
      nexj.core.monitoring.Counter
      nexj.core.monitoring.Flag
      nexj.core.monitoring.Value
   )
)
...
; Supports macros from Exec steps in Main.upgrade such as ${table:}, etc.
...
(define (sql-execute ds sql . bindParams)
   ...
   (try
      (let* 
         (
            (datasource (if (string? ds) (((invocation-context)'metadata)'getDataSource ds) ds))
            (adapter ((datasource'component)'getInstance (invocation-context)))
            (schemaManager (adapter'createSchemaManager))
            (ordinal 0)
         )
         ;Process the ${} macros:
         (set! sql (schemaManager'getExecSQL sql (datasource'schema) ()))
